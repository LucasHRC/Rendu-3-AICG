...
export function createSystemControls() {
  ...
  panel.innerHTML = `
    <div class="flex items-center justify-between mb-4">
      ...
      <button id="help-tutorial-btn" class="px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors" title="Démarrer le tutoriel">
        Aide
      </button>
    </div>
...
function setupControlEvents(panel) {
  ...
  const helpBtn = panel.querySelector('#help-tutorial-btn');
  helpBtn?.addEventListener('click', () => {
    startTutorial();
    addLog('info', 'Tutoriel démarré');
  });
}
